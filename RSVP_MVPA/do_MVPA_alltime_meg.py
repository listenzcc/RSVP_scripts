...
preds_xdawn = np.empty([len(labels), len(w_start)+1])
...
for train, test in cv.split(epochs_data, labels):
    ...
    preds_xdawn[test, len(w_start)] = clf.predict(normalize_pipeline.transform(data_test_))
    ...
    for j, start in enumerate(w_start):
        ...
        clf.fit(normalize_pipeline.fit_transform(data_train_), labels[train])
        preds_xdawn[test, j] = clf.predict(normalize_pipeline.transform(data_test_))


'''
# Function: Save report into file.
'''
fpath = os.path.join(results_dir, 'accs_meg_%s.txt' % (time_stamp))

report = classification_report(preds_xdawn[:, len(w_start)], labels,
...
for j, start in enumerate(w_start):
    print(j)
    report = classification_report(preds_xdawn[:, j], labels,
                                   target_names=['odd', 'norm'])
